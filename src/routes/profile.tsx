...
function ProfilePage() {
  ...
  useEffect(() => {
    setFarmer(getData<Farmer>("farmsmart_farmer") ?? { name: "किसान भाई", location: "India", landAcres: 2 });
    ...
    <div className="space-y-4">
      <section className="glass-card rounded-2xl p-4">
          ...
          <div className="flex-1">
            {edit ? (
              <div className="space-y-1">
                <input className="w-full min-touch px-2 rounded-lg border bg-background text-sm" value={farmer.name} onChange={(e) => setFarmer({ ...farmer, name: e.target.value })} placeholder="नाम" />
                <input className="w-full min-touch px-2 rounded-lg border bg-background text-sm" value={farmer.location} onChange={(e) => setFarmer({ ...farmer, location: e.target.value })} placeholder="स्थान" />
                <input type="number" className="w-full min-touch px-2 rounded-lg border bg-background text-sm" value={farmer.landAcres} onChange={(e) => setFarmer({ ...farmer, landAcres: +e.target.value })} placeholder="एकड़" />
              </div>
            ) : (
              <>
                <div className="font-bold text-lg text-primary">{farmer.name}</div>
                <div className="text-xs text-muted-foreground flex items-center gap-1"><MapPin className="w-3 h-3" />{farmer.location} • {farmer.landAcres} एकड़</div>
              </>
            )}
          </div>
        ...
        <div className="flex items-center gap-1.5 text-[10px] text-success font-semibold">
          ...
          Storage Ready • offline भी काम
        </div>
      </section>

      <section className="glass-card rounded-2xl p-4">
        <h3 className="font-bold text-primary mb-3"><Bi k="scanHistory" /></h3>
        {history.length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-4">कोई स्कैन नहीं अभी तक</p>
        ) : (
            ...
            {history.map((h) => (
              <div key={h.id} className="flex items-center gap-3 p-2 rounded-xl bg-secondary/40">
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-semibold text-sm truncate">{diseaseHindi(h.cropType + "___" + h.diseaseName.replace(/ /g, "_"))}</div>
                  <div className="text-[10px] text-muted-foreground">{new Date(h.date).toLocaleDateString("hi-IN")} • {h.confidence}%</div>
                </div>
      ...
      <section className="glass-card rounded-2xl p-4">
        <h3 className="font-bold text-primary mb-3"><Bi k="activeTreatments" /></h3>
        {reminders.filter((r) => !r.done).length === 0 ? (
          <p className="text-xs text-muted-foreground text-center py-4">कोई चालू उपचार नहीं</p>
        ) : (
          <div className="space-y-2">
              ...
              return (
                <div key={r.id} className="flex items-center justify-between gap-2 p-3 rounded-xl bg-warning/15">
                  <div className="flex-1">
                    ...
                    <div className="text-[10px] text-muted-foreground">{days} दिन बाद फॉलो-अप</div>
                  </div>
      ...
      <section className="glass-card rounded-2xl p-4 text-center text-xs text-muted-foreground">
        ...
        सारा डेटा आपके फ़ोन में है — कहीं नहीं भेजा जाता
      </section>
    </div>
  );
}
